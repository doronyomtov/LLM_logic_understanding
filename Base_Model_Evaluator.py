# File: llama3_evaluator_base.py
import torch, pandas as pd, numpy as np, json, matplotlib.pyplot as plt
from transformers import AutoModelForCausalLM, AutoTokenizer
from tqdm import tqdm

class Base_Model_Evaluator: 
    def __init__(self, base_model_path, csv_path, start_idx=1000, num_rows=None, confidence_threshold=0.2):
        """
        Initialize the Base_Model_Evaluator with model path, CSV path, and other parameters.
        :param base_model_path: Path to the base model directory.
        :param csv_path: Path to the CSV file containing the evaluation data.
        :param start_idx: Starting index for evaluation (default is 1000).
        :param num_rows: Number of rows to evaluate (default is None, which means all rows).
        ...
        print("--- Base Evaluator Initialized ---")
    ...
    def _prepare_token_ids(self):
        """Prepare token IDs for allowed tokens and 'yes'/'no' tokens."""
    ...
    def _get_constrained_answer(self, prompt):
        """
        Get the model's answer to the prompt, constrained to 'yes' and 'no' tokens.
        :param prompt: The input prompt for the model.
        :return: A tuple containing the answer, confidence score, uncertainty flag, and probabilities for 'yes' and 'no'.
        """
    ...
    def evaluate(self, output_path='evaluation_results_base.csv', plot_path='confidence_distribution_base.png'): 
        ...
        except Exception as e: print(f"Error saving base CSV: {e}") 
        self._print_summary(stats, "Base Model") 
        if stats["confidence_scores"]: self._plot_confidence_histogram(stats["confidence_scores"], "Base Model", plot_path)
        return stats
